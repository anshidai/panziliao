...
CREATE TABLE `pzl_index_url` (
  ...
  `addtime` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '添加时间 格式：时间戳',
  `adddate` varchar(50)  utf8_unicode_ci NOT NULL DEFAULT '' COMMENT '添加时间 格式：日期格式',
...
CREATE TABLE `pzl_res_detail` (
  ...
  `cj_url` varchar(255) COLLATE utf8_unicode_ci NOT NULL DEFAULT '' COMMENT '采集url',
  `cj_url_index` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '采集url表_id',
  PRIMARY KEY (`_id`),
  KEY `catid` (`catid`),
  KEY `userid` (`userid`),
  KEY `addtime` (`addtime`)
